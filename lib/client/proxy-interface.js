  ...
  static _fromXml (object, xml) {
    ...
    if (Array.isArray(xml.method)) {
      for (const m of xml.method) {
        ...
        // choose a unique property name on iface (Foo, Foo1, Foo2, ...)
        let propName = method.name;
        let counter  = 1;
        while (Object.prototype.hasOwnProperty.call(iface, propName)) {
          propName = method.name + String(counter);
          counter += 1;
        }

        // expose which property name was chosen for introspection
        method.propertyName = propName;

        iface[propName] = function (...args) {
          const objArgs = [
            name,
            method.name,
            method.inSignature,
